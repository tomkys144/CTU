...
for i=1:wnum,
  ...
  %filtration
  Sframe = Sframe./H; % musí být spočítaná na obě dvě strany kvůli komplexním číslům
  yframe = ifft(Sframe) ;
...
nr = out - sig;
P_nr = mean(nr.^2);
% výkon šumu po přidání šumu do zkresleného signálu a následné zpětné
% filtrace
%output SNR
...
for i=1:wnum,
  ...
  %filtration
  % když šum je větší, hodnota ve jmen. začne být významnější
  % celý zlomk půjde k nule, v tomhle okamžiku se to zatlumí bez ohledu na
  % signál
  % Sn - spektrální výkon šumu
  % Gs - spektrální výkon segmentu signálu
...
% filtrace
%output SNR
SNR_inv2WF = 10*log10(P_orig/P_nr);
